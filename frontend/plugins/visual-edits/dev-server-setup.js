...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow all healthgrow.online subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*healthgrow\.online$/)) {
        return true;
      }
      ...
      try {
        ...
        for (const change of variableEditChanges) {
          ...
          if (result.success) {
            ...
            try {
              execSync(`git -c user.name="visual-edit" -c user.email="support@healthgrow.online" add "${result.file}"`);
              execSync(`git -c user.name="visual-edit" -c user.email="support@healthgrow.online" commit -m "visual_edit_variable_${timestamp}"`);
            } catch (gitError) {
